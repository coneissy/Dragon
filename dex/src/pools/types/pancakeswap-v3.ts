...
export class PancakeV3Pool implements IPoolMath {
  ...
  applyInitialResults(results: InitialStateResult[]): boolean {
    ...
    if (slot0Res.status !== "success" || liqRes.status !== "success") {
      logger.warn({ pool: this.config.address }, "Failed to load PancakeV3 state");
      return false;
    }
  ...
  handleMintEvent(log: Log): void {
    const decoded = decodeEventLog({ abi: PANCAKE_V3_MINT_ABI, data: log.data, topics: log.topics });
  ...
  handleBurnEvent(log: Log): void {
    const decoded = decodeEventLog({ abi: PANCAKE_V3_BURN_ABI, data: log.data, topics: log.topics });
    const { tickLower, tickUpper, amount } = decoded.args;
    this.updateTickLiquidity(Number(tickLower), Number(tickUpper), -BigInt(amount));
  }
...
export const PANCAKE_V3_SWAP_EVENT = parseAbiItem(
  "event Swap(address indexed sender, address indexed recipient, int256 amount0, int256 amount1, uint160 sqrtPriceX96, uint128 liquidity, int24 tick, uint128 protocolFeesToken0, uint128 protocolFeesToken1)",
);

const PANCAKE_V3_MINT_ABI = [
  {
    type: "event",
    name: "Mint",
    inputs: [
